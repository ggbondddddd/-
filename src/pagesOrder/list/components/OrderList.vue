...
import type { triggerRef } from 'vue'
// 获取屏幕边界到安全区域距离
const { safeAreaInsets } = uni.getSystemInfoSync()

// 定义props接收父组件list传过来的数据
...
// // 定义一个数据，当pages到了最后一个由fasle变为true
const finish = ref(false)
// 获取订单列表
const orderList = ref<OrderItem[]>([])
const getMemberOrderData = async () => {
  // 这个请求参数的接口很精妙，通过参数orderState来判断tabs栏每一栏展示的数据
  ...
  // 由于要触底显示下一页数据，所以不能直接赋值而应该追加,而且要展开数组再追加
...
const onOrderPay = async (id: string) => {
  ...
  // 找到orderList也就是所有订单中支付了的那个订单，让那个订单的orderstate切换到代发货
  const order = orderList.value.find((v) => v.id === id)
  order!.orderState = OrderState.DaiFaHuo
}

// 点击确认收货
const enterRecieve = async (id: string, index: number) => {
  const res = await putMemberOrderReceiptByIdAPI(id)
  orderList.value[index].orderState = res.result.orderState
  uni.showToast({ title: '确认收货' })
}

// 当下拉应该重置数据，让page从零开始，且数据也从第一页开始
const resetData = () => {
  ;(queryParams.page = 1), (orderList.value = []), (finish.value = false)
}
...
<template>
  ...
  <scroll-view
    ...
    class="orders"
  >
    <view class="card" v-for="(order, index) in orderList" :key="order.id">
      <!-- 订单信息 -->
      <view class="status">
      ...
      <view class="action">
        ...
        <template v-else>
          ...
          <view
            ...
            @tap="enterRecieve(order.id, index)"
            >确认收货</view
          >
        </template>
      </view>
