...
// uniapp获取页面参数的写法，这是页面的vue文件，可以获取到你再HotPanel.vue里面路由跳转时候携带的query参数
...
//推荐选项    <(SubTypeItem & {finish:boolean})[]> 意思是里面两个类型&,作为这个数组的类型，然后整体为一个泛型，这里()应该是优先级
// 而finish需要加上?就是这个属性是可有可无的，因为后端返回数据里面是没有这个属性的，这个属性是后端数据返回以后我们加上处理底部文字
const subTypes = ref<(SubTypeItem & { finish?: boolean })[]>([])
// 接收点击的那个index，当你index=这个index高亮显示
...
const onScrolltolower = async () => {
  ...
  // 追加到老数据里面，这里我没有用触底回调函数里面新声明的那个变量，我以为那个值和这个值可能不一起，其实是一样的，应该也是浅拷贝的关系
  subTypes.value[activeIndex.value].goodsItems.items.push(...newsubType.goodsItems.items)
}
</script>
